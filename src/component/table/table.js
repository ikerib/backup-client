...
class Table extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      JSON.stringify(this.props.selectedFs) !==
      JSON.stringify(nextProps.selectedFs)
    ) {
  ...
  render() {
    const lastDir = this.props.selectedFs.slice(
      0,
      this.props.selectedFs.lastIndexOf("/")
    );
    const item = {
      path: lastDir,
      name: "..",
    ...
    if (this.state.loading) {
      return <div>Datuak eskuratzen...</div>;
    } else {
      const nirefs = this.state.myfs.children;
      let newdir = "";
      if (
        this.props.selectedFs !== undefined &&
        this.props.selectedFs !== null
      ) {
          ...
          <table className="table table-condensed table-bordered table-hover table-striped tableNoFilesElse">
            ...
            <tbody>
              <tr>
                ...
                <td>{(item.size / 1024).toFixed(2)} Kb</td>
                <td>{item.mode}</td>
                <td>{Moment(item.birthtime).format("YYYY-MM-DD HH:mm:ss")}</td>
                <td>{Moment(item.mtime).format("YYYY-MM-DD HH:mm:ss")}</td>
              </tr>
              <tr>
                <td colSpan="6">Ez dago daturik</td>
